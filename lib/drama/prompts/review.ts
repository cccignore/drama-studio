...
function buildHardRules(state: DramaState): string {
  const b = getEpisodeBudget(state.mode);
  const timeLine =
    state.mode === "overseas"
      ? `R10 时长合理：出海单集应在 ${b.secMin}-${b.secMax}s、英文台词 ${b.enMin}-${b.enMax} 词、中文场记约 ${b.cnMin}-${b.cnMax} 字；明显超纲或不足都算问题。`
      : `R10 时长合理：国内单集应在 ${b.secMin}-${b.secMax}s、约 ${b.cnMin}-${b.cnMax} 字；明显超纲或不足都算问题。`;
  const lineLine = `R11 台词长度：每句台词原则上 ≤${b.lineMaxCn} 字（中文）/ ≤${b.lineMaxEn} 词（英文）；超长台词必须按 7 级拆解（拆句 / 加打断 / 配物件 / 切反应 / 改对抗 / 删废话 / 加位置变化）处理。`;
  return `【硬规则清单（issue.rule 必须引用其中一条）】
...
export function buildReviewMessages(
  ...
  const HARD_RULES = buildHardRules(state);
  const user = [
